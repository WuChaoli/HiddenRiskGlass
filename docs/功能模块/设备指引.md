...
4. 若返回命中，页面切到 `RESULT / PROMPT`，短暂停留后自动进入详情拉取。
5. 页面调用深度分析接口拉取检查重点文本；用户也可确认立即进入。
6. 页面切到 `RESULT / DETAIL`，展示检查品说明和检查重点。
7. 用户返回时回到主菜单首页。
8. 用户也可以在检测态说“实时分析””隐患拍照””结束任务”跳到对应功能。
...
| `DeviceGuideActivity` | `RESULT / PROMPT` | 远端判定返回命中 | 停留约 2 秒自动拉详情 / 确认立即拉详情 / 返回菜单 | 底部提示由弹窗承载 |
| `DeviceGuideActivity` | `RESULT / DETAIL` | 命中提示自动触发或用户确认后详情拉取中或成功 | 返回菜单 | 展示卡片文本 |
...
- 周期性抓取帧并调用远端判定。
- 在命中后短暂停留提示态，并自动拉取详情；用户也可确认立即拉取详情。
- 拉取并展示检查重点详情。
...
- 语音”隐患拍照”：进入 `HazardRecordActivity`。
...
| --- | --- | --- | --- | --- |
| `Confirm` | 单击 | `RESULT / PROMPT` | `DeviceGuideActivity.buildInputActions()` | 立即拉取详情 |
...
| `隐患拍照` | `yin huan pai zhao` | 跳转隐患拍照 | `DETECTING` | `DeviceGuideActivity.buildInputActions()` |
...
| `DeviceGuideActivity / DETECTING` | 语音”隐患拍照” | `HazardRecordActivity` | 无 |
...
- 接口用途：在命中提示后拉取检查品说明和检查重点。
- 触发时机：`RESULT / PROMPT` 停留约 2 秒后自动触发；用户确认可立即触发。
- 调用入口代码：
...
- [ ] 命中后先显示提示态，约 2 秒自动进入详情拉取
- [ ] 命中提示态下点击或语音“确认 / 确定 / 继续”会立即进入详情拉取，且不会重复请求
- [ ] 语音”实时分析 / 隐患拍照 / 结束任务”只进入目标功能首页
