...
- 语音“结束任务 / 结束”：进入 `InspectionEndReportActivity`。
- 分析态确认上传成功后回到空闲态。
...
| `结束` | `jie shu` | 结束任务 | `IDLE` | `HazardRecordActivity.buildInputActions()` |
...
| `HazardRecordActivity / IDLE` | 语音“结束任务 / 结束” | `InspectionEndReportActivity` | 无 |
