...
5. 自动在线链路或本地备用链路命中隐患后，页面切到 `STREAM_RESPONSE` 结果态，先展示描述页。
6. 用户在描述页确认后，先触发手机端同步，再进入整改建议页。
7. 用户在整改建议页确认或取消后，返回检测态继续巡检。
8. 用户在检测态说“结束”或“结束巡查”时，跳转到结束巡检页。
...
- [ ] 自动在线检测请求是否仍为 `ctype = 1`，并且超时、单飞和失败阈值逻辑未漂移。
