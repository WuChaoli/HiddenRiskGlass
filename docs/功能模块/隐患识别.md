...
- 检测态“结束 / 结束巡查 / 结束识患”进入 `InspectionEndReportActivity`。
...
| `结束` | `jie shu` | `ai_detecting_finish` | `DETECTING` | 进入结束巡检页 |
| `结速` | `jie su` | `ai_detecting_finish` | `DETECTING` | 进入结束巡检页 |
| `结束巡查` | `jie shu xun cha` | `ai_detecting_finish` | `DETECTING` | 进入结束巡检页 |
| `结束识患` | `jie shu shi huan` | `ai_detecting_finish` | `DETECTING` | 进入结束巡检页 |
...
| `AiInspectionActivity / DETECTING` | `结束` 系列语音 | `InspectionEndReportActivity` | 无 |
...
- 调用入口：
  ...
  - 场景识别 lane：`AiArSseService.identifySceneHazard()`
...
- [ ] 自动在线物品识别是否仍为 `ctype = 1`，场景识别是否仍为 `ctype = 2`。
- [ ] 物品识别是否按 `onlineDetectIntervalMs` 触发，场景识别是否按 `onlineSceneDetectIntervalMs` 触发。
- [ ] 任一路返回“是”后，是否停止双 lane 循环并取消另一条在途检测。
