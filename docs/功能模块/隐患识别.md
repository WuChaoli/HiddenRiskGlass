...
关联旅程图：[总体旅程图/隐患识别旅程图.md](../总体旅程图/隐患识别旅程图.md)
...
5. 自动在线链路运行物品识别 lane；返回“是”即停止在线检测并进入详情链路。
...
- 从 `AiInspectionMenuActivity` 点击”实时分析”进入。
...
- 检测态”隐患拍照”进入 `HazardRecordActivity`。
...
- `ResolvedHazardContent` 进入本地上传前，由 `LocalHazardUploadItemBuilder` 组装上传项：跳过空 `hidNum`，按 `hidNum` 去重，重复时保留首条；`hidLevel` 有值时原样提交。
...
| `隐患拍照` | `yin huan pai zhao` | `ai_detecting_hazard_record` | `DETECTING` | 跳转 `HazardRecordActivity` |
...
| --- | --- | --- | --- |
| `AiInspectionMenuActivity` | 点击”实时分析”且 `InspectionSession.isInitialized == true` | `AiInspectionActivity` | 无 |
| `AiInspectionMenuActivity` | 点击”实时分析”且 `InspectionSession.isInitialized == false` | `InspectionLoadingActivity` | 初始化失败留在本页 `ERROR` |
...
| `AiInspectionActivity / DETECTING` | `隐患拍照` | `HazardRecordActivity` | 无 |
...
- 接口地址来源：`InspectionConfigRepository.get().network.aiAutoApi.url`
- 默认地址：`http://183.147.142.133:10010/ai/auto`
- 请求协议：
  - JSON `POST`
  - 物品识别：`/ai/auto`
  - 深度分析：`/ai/deep` (SSE, `Accept: text/event-stream`)
  - 设备指引：`/ai/device` (SSE, `Accept: text/event-stream`)
  - 环境隐患识别：`/ai/general`
- 请求体字段：
  - `task_id`、`stream`、`image`（物品识别 / 深度分析）
  - `task_id`、`stream`、`text`（在线建议 / 设备指引）
- 成功后：
...
- 关键日志锚点：
  - `openStream requestStart`
   - `manual ai/deep opened`
   - `manual ai/deep closed`
  - `inspection guide opened`
  - `inspection guide failed`
  - `handle_online_detail_success`
- 这些锚点及自动在线识别的状态切换、请求发起、失败和收口日志会同步进入 `AppFileLogger` 文件日志；拉取方式见 `docs/公共能力/日志系统.md`。
...
- 接口地址来源：
  - 唯一接口：`enterpriseQrPayload.apiBaseUrl`
  - 路径：`<baseUrl>/smartGlasses/pushHidDanger`（若 baseUrl 已含 `/smartGlasses` 则为 `<baseUrl>/pushHidDanger`）
- 提交策略：
  - 单端点提交，通过 `InspectionRetryExecutor` 最多重试 4 次（1s / 2s / 3s 递增延迟）
- 注意：
  - 业务错误码（如 `100001` "检查人不存在"）也会触发重试，但每次都会返回相同失败。若排查发现是业务错误，应从 QR 码字段或 userId 映射入手，不应靠重试解决。
- 请求体字段：
  - `authCode`
  - `objectId`
  - `userId`
...
- 失败后：
  ...
  - 停留在描述页
  - 底部提示区显示后端返回的错误文案（如 `"检查人不存在"`）
  - 若为业务错误码（非网络错误），所有 4 次重试均返回相同结果
- 关键日志锚点：
  ...
  - `local save failed`
- 这些锚点及上传前置校验、成功、失败收口日志会同步进入 `AppFileLogger` 文件日志；拉取方式见 `docs/公共能力/日志系统.md`。
...
- [ ] 自动在线物品识别经 `/ai/auto` 必跑，场景识别经 `/ai/general`，设备指引经 `/ai/device`。
- [ ] 物品识别是否按 `onlineDetectIntervalMs`（默认 `500ms`）触发，并发池是否限制在 `onlineDetectConcurrencyLimit`（默认 5）以内。
- [ ] 物品识别返回“是”后，是否停止在线检测并进入详情链路。
- [ ] 在线检测 lane 是否都直接 fresh capture，而不是等待本地 NCNN shared frame。
- [ ] 在线 description 页是否保留原始文字流，保存 / 上传链路是否仍使用解析后的结构化内容。
- [ ] 本地隐患上传项是否跳过空隐患编号，并按隐患编号去重；隐患等级 `2` 是否仍会提交。
- [ ] SSE done 事件是否兼容 `data=[DONE]` 和 `type=done data=["DONE"]`，且不再误报“在线识别事件不是合法 JSON”。
- [ ] 手动分析 / 自动详情请求使用 `/ai/deep`，在线建议使用 `/ai/device`。
- [ ] `/ai/auto`、`/ai/deep`、`/ai/device`、`/ai/general` 地址、超时和 `saveResultApi` 配置是否仍来自 `InspectionConfigRepository`。
- [ ] 本地隐患同步时，是否仅通过主 URL 单端点提交，重试策略是否正常生效。
- [ ] 描述页确认后是否先做同步，再进入建议页。
- [ ] 同步失败时是否留在描述页，并把错误写到底部提示区。
- [ ] `InspectionWorkflowSession` 是否在描述页写入 capture、detection、analysis，在同步结果回调里写入 save outcome。
- [ ] `userId` 是否从 QR 码正确解析（`parseEnterpriseQrPayload` 的 position 2），是否与 `authCode` 串值不对。若服务端返回 `检查人不存在`，先逐字段打印 QR 解析结果再排查后端。
