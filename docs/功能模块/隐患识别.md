...
- 默认地址：`http://183.147.142.133:5000/ai/ar`
- 请求协议：
  - SSE `POST`
...
- lane 节拍：
  - 物品识别：`onlineDetectIntervalMs`，默认 `500ms`
- 触发时机：
  - 检测态自动巡检周期触发。
  - 物品识别 lane 内允许最多 5 个并发请求（由 `onlineDetectConcurrencyLimit` 控制），每个请求独立超时，超时时间由 `network.aiArApi.detectTimeoutMs`（默认 `1500ms`）控制。
  - 物品识别 lane 必跑；场景识别 lane 当前不作为运行链路参与调用，`ctype = 2` 保留给检查指引。
...
- 成功后：
  ...
  - 仅物品识别 lane 参与自动在线检测，返回“是”即停止在线检测。
...
- 关键日志锚点：
  ...
  - `openStream requestStart ctype=3`
  - `openStream received done sentinel`
  - `detect success lane=...`
  - `detect failure lane=...`
  - `detect timeout`
  - `submitDetection accepted lane=... activePoolSize=... concurrencyLimit=...`
  - `submitDetection droppedBusy lane=... activePoolSize=... concurrencyLimit=...`
...
- 接口地址来源：
  ...
  - 在线建议：同样走 `network.aiArApi.url`，但 `ctype = 2`
- 请求协议：
  - `ctype = 0`：对图像做深度分析
  - `ctype = 2`：根据描述文本生成检查指引
- 请求体字段：
...
- [ ] 自动在线物品识别是否为 `ctype = 3` 且必跑，场景识别 lane 是否未参与运行链路。
...
- [ ] 手动分析 / 自动详情请求是否仍为 `ctype = 0`，在线建议是否仍为 `ctype = 2`。
