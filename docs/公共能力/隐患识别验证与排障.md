...
当前巡检页远端主链只运行物品识别 lane；场景识别 lane 当前不作为运行链路参与调用，`ctype=2` 保留给检查指引：

- 物品识别：`lane=item`，`ctype=3`，默认 500ms 一帧
- 检查指引：`ctype=2`，请求字段为 `text`
...
- `openStream requestStart ctype=2`，仅检查指引请求时应出现
...
- 物品识别返回“是”后，应停止在线检测，再进入 `ctype=0` 在线详情。
