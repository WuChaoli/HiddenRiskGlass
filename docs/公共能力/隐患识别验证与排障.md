...
当前巡检页远端主链包含两条在线 lane：

- 物品识别：`lane=item`，`ctype=1`，默认 1 秒一帧
- 场景识别：`lane=scene`，`ctype=2`，默认 3 秒一帧
...
- 两条 lane 都应直接 fresh capture，不应出现在线链路等待本地 shared frame 的判断。
- 任一路返回“是”后，应停止双 lane 循环并取消另一条在途检测，再进入 `ctype=0` 在线详情。
- 任一路返回“否”只结束该 lane 当前请求，不应主动取消另一条 lane。
