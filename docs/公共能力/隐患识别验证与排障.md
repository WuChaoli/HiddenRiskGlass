...
当前巡检页远端主链至少包含物品识别 lane；场景识别 lane 受 `enableOnlineSceneHazardDetection` 控制，基础配置默认关闭：

- 物品识别：`lane=item`，`ctype=1`，默认 1 秒一帧
- 场景识别：`lane=scene`，`ctype=2`，启用后默认 3 秒一帧

真机排查时优先过滤：

- `post online inference loop lane=item`
- `post online inference loop lane=scene`，仅场景识别启用时应出现
- `selected online frame lane=item`
- `selected online frame lane=scene`，仅场景识别启用时应出现
- `openStream requestStart ctype=1`
- `openStream requestStart ctype=2`，仅场景识别启用时应出现
...
- 场景识别启用时，任一路返回“是”后，应停止双 lane 循环并取消另一条在途检测，再进入 `ctype=0` 在线详情。
- 任一路返回“否”只结束该 lane 当前请求，不应主动取消其他 lane。
