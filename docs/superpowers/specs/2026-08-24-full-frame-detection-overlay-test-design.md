...
> **2026-08-24 真机修订（覆盖下文旧尺寸链路）：** Glass SDK 不支持 `960×1280` NV21，且 `3024×4032` NV21 + Surface 会触发设备内存压力。最终测试链路使用设备明确支持的 `1200×1600@15fps`、`zoomLevel=1`，NV21 不裁切、不缩放，原尺寸编码后直接调用 `/auto`。返回 BBox 已位于 `1200×1600` 坐标系；利用归一化 1m 标定窗口直接投影到 `480×640`，不再经过 `3024×4032` 中间坐标。归一化映射与旧两步映射在数学上等价，并由 `FullFrameOverlayMapperTest` 覆盖。
