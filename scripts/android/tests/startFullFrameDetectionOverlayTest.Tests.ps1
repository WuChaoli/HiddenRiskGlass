...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) "glassdemo-full-frame-overlay-test-$PID"
New-Item -ItemType Directory -Path $tempRoot | Out-Null

try {
    Copy-Item -LiteralPath (Join-Path $androidScriptRoot 'start-full-frame-detection-overlay-test.ps1') `
...
'@ | Set-Content -LiteralPath (Join-Path $tempRoot 'start-activity.ps1') -Encoding UTF8
    ...
    $defaultResult = (& (Join-Path $tempRoot 'start-full-frame-detection-overlay-test.ps1')) |
        ConvertFrom-Json
    ...
    $serialResult = (& (Join-Path $tempRoot 'start-full-frame-detection-overlay-test.ps1') `
        -Serial 'glass-001') | ConvertFrom-Json
    Assert-Equal 'glass-001' $serialResult.Serial 'explicit serial'

    Write-Host 'PASS start-full-frame-detection-overlay-test.ps1 tests'
} finally {
